...
def split_list(value):
    ...
    if value.startswith('(') and value.endswith(')'):
        value = value[1:-1]
    return [a or b for a, b in re.findall(r'"([^"]*)"|(\S+)', value) if (a or b)]
...
def discover_header_dirs(core_dir):
    ...
    for root, dirs, files in os.walk(core_dir):
        ...
        if not headers:
            continue
        if any(f in SYSTEM_HEADER_NAMES for f in headers):
            # Skip it entirely rather than adding it and hoping nothing
            # includes the shadowed name.
            continue
        found.add(root)
    return sorted(found)
...
def main():
    ...
    if target_id is None:
        targets = re.findall(r'^\t\t([0-9A-Za-z]{20,}) /\* ([^*]+) \*/ = \{\n\t\t\tisa = PBXNativeTarget;',
                             text, re.M)
        if len(targets) == 1:
            target_id, name = targets[0]
        else:
            print(f'error: cannot pick a target for {args.core}: '
                  f'{", ".join(n for _, n in targets)}', file=sys.stderr)
            return 1

    settings = resolved_settings(project_path, args.core, core_dir)
    file_paths = file_reference_paths(text)

    # Source paths are relative to the project directory.
    sources = []
    for path, flags in phase_files(text, target_id, 'PBXSourcesBuildPhase', file_paths):
    ...
    discovered = discover_header_dirs(core_dir)
