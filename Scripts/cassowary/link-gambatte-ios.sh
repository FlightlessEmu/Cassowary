...
#
# Run Scripts/cassowary/build-gambatte-ios.sh first; this picks up the objects it
...
#
# Usage:
#   Scripts/cassowary/link-gambatte-ios.sh [--device]
...
OBJECTS_DIR="build/cassowary-gambatte-${MODE}"
PLUGIN_DIR="build/cassowary-plugins-${MODE}/Gambatte.oecoreplugin"

if [[ ! -d "$OBJECTS_DIR" ]]; then
  print -u2 -- "error: no objects in $OBJECTS_DIR; run Scripts/cassowary/build-gambatte-ios.sh first"
  exit 1
fi
...
for framework in "${FRAMEWORKS[@]}"; do
  if [[ ! -d "$framework" ]]; then
    ...
    print -u2 -- "       see Scripts/cassowary/build-cassowary.sh for the right invocation"
    exit 1
  fi
done
